...
import { VOUCHER_LABEL, usesVcr, type EmployeeLevel } from "./constants";
...
export async function buildPayslip(employeeId: string, month: string): Promise<Payslip | null> {
  ...
  if (!vcr && employee.salary && employee.salary > 0) {
    draft.push({ id: null, date: null, description: "GAJI POKOK", qty: null, debit: employee.salary, credit: 0 });
  }
  if (kasbonTotal > 0) {
